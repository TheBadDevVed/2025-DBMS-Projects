...
class _KYCLivePhotoPageState extends State<KYCLivePhotoPage> {
  ...
  String? _userId; // UID generated internally
  ...
  Future<void> _generateUid() async {
    try {
      ...
      if (user != null) {
        ...
        setState(() => _userId = anonUser.user?.uid);
      }
    } catch (e) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Failed to generate UID: $e')));
    }
  }
  ...
  void _handleSubmit(BuildContext context) async {
    if (!_photoTaken || _userId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please take a live photo first or wait for UID.'),
    ...
    try {
      const String PLACEHOLDER_URL =
          "https://teenpay.com/storage/kyc-placeholder.jpg";

      final kycData = {
        'userId': userId,
        'submissionDate': FieldValue.serverTimestamp(),
        'kycStatus': 'DETAILS SUBMITTED (Images Pending Demo)',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            '✅ KYC Submission Successful! Data written to Firestore.',
          ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Submission Failed: Firestore Write Error: $e'),
          backgroundColor: Colors.red,
        ),
      );
    } finally {
  ...
  Widget _buildCameraUploadArea() {
    return Center(
      child: GestureDetector(
        onTap: () => _captureLivePhoto(context),
        child: Container(
          width: double.infinity,
          height: 240,
          decoration: BoxDecoration(
